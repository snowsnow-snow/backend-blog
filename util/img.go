...
func ReadExif(path string, fileName string, fileType string) *models.ImgInfo {
	...
	if err != nil {
		file, err = os.Open(fullPath)
		setPixelXAndYDimensionByBounds(imgInfo, file)
		err = Compress(path, fileName, fileType, 1, file)
		if err != nil {
			logger.Error.Println("Compress file error", err)
		}
		logger.Error.Println("Read exif info error: ", err, string(debug.Stack()))
		return imgInfo
	}
	orientation := ReadOrientation(exifInfos)
	err = Compress(path, fileName, fileType, orientation, file)
	if err != nil {
		logger.Error.Println("Compress file error", err)
	}
	// 图片宽高信息
...
func setISO(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
...
func setExposureProgram(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	...
	info := models.ExposureProgramMap[uint16(dataType)]
	imgInfo.ExposureProgram = info.ExposureProgram
	imgInfo.ExposureProgramZhCN = info.ExposureProgramZhCN
}

func setFocalLength(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
...
func setPixelXAndYDimensionByBounds(imgInfo *models.ImgInfo, file *os.File) {
	...
	bounds := img.Bounds()
	imgInfo.PixelXDimension = bounds.Dx()
	imgInfo.PixelYDimension = bounds.Dy()
}

func setPixelXAndYDimension(imgInfo *models.ImgInfo, exifInfos *exif.Exif, orientation int) {
	pixelXDimension, err := exifInfos.Get(exif.PixelXDimension)
	if err != nil {
		logger.Warn.Println(err.Error())
		return
	}
	pixelXDimensionVal, err := pixelXDimension.Int64(0)
	if err != nil {
		logger.Warn.Println(err.Error())
		return
	}
	pixelYDimension, err := exifInfos.Get(exif.PixelYDimension)
	if err != nil {
		logger.Warn.Println(err.Error())
		return
	}
	pixelYDimensionVal, err := pixelYDimension.Int64(0)
	if err != nil {
		logger.Warn.Println(err.Error())
		return
	}
	...
	if orientation == 8 {
		imgInfo.PixelXDimension = int(pixelYDimensionVal)
		imgInfo.PixelYDimension = int(pixelXDimensionVal)
	} else {
		imgInfo.PixelXDimension = int(pixelXDimensionVal)
		imgInfo.PixelYDimension = int(pixelYDimensionVal)
	}
}
func setShutterSpeedValue(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	...
	imgInfo.ShutterSpeedValue = formatFloat(float64(shutterSpeedValueNum)/float64(shutterSpeedValueDen), 2)
}

func setExposureTime(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	...
	if sameDigits(exposureTimeNum, exposureTimeDen) {
		...
		imgInfo.ExposureTime = formatFloat(float64(exposureTimeNum)/10, 0) + "/" + formatFloat(float64(exposureTimeDen)/10, 0)
	}
}

func setApertureValue(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	...
	imgInfo.ApertureValue = formatFloat(float64(apertureValueNum)/float64(apertureValueDen), 2)
}
func setFNumber(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	...
	imgInfo.FNumber = "f/" + formatFloat(float64(fNumberNum)/float64(fNumberDen), 2)
}
...
func setLatitudeAndLongitude(imgInfo *models.ImgInfo, exifInfos *exif.Exif) {
	// 获取经度
